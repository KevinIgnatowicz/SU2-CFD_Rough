...
void CTurbSASolver::Postprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh) {
  ...
  for (iPoint = 0; iPoint < nPoint; iPoint ++) {
    ...
    if (rug_heat_aupoix) {
      ...
      roughness_height = (geometry->node[iPoint]->GetRoughness_Height());
        if (ks > 1e-10)  {
    		...
    		/*--- approximate nu_hat=kappa*utau*d ---*/
        		...
        		ksplus = ks*utau/nu;
        		delta_uplus = 1./kappa*log(1.0+ksplus/exp(1.3325));
      ...
      roughness_height = geometry->node[iPoint]->GetRoughness_Height();
        ...
        if (ks > 1e-10)  {
    		...
    		/*--- approximate nu_hat=kappa*utau*d ---*/
        		...
        		//if ( deltaPrT > 0.5 ) cout << "deltaPrT hkc" << deltaPrT << " , " << ksplus << endl;
        		} else {
        		deltaPrT=0.0;
        		}
    		node[iPoint]->SetdeltaPrT(deltaPrT);

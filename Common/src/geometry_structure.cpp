...
void CPhysicalGeometry::SetRough_Zone(CConfig *config) {
  ...
  su2double LE_Radius = 0.0, Ac = 0.0, n0 = 0.0, abs_curv = 0.0, step = 0.0, rough_lim = 0.0;
  su2double smooth_lim = 0.0, roughness_max = 0.0, roughness_height = 0.0, dist = 0.0;
  su2double X_coord = 0.0, Y_coord = 0.0, Z_coord = 0.0, X_wall_coord = 0.0, Y_wall_coord = 0.0, Z_wall_coord = 0.0, AoA_offset = 0.0, X_pos = 0.0;
  su2double chord = config->GetLength_Reynolds();
  bool variable_roughness = (config->GetVariable_Roughness() == true);
  ...
  if( WallADT.IsEmpty() ) {
    ...
    for(unsigned long iPoint=0; iPoint<GetnPoint(); ++iPoint){
      node[iPoint]->SetRough(0.0);
      node[iPoint]->SetRoughness_Height(0.0);
    }
  }
  else {
    ...
    for(unsigned long iPoint=0; iPoint<GetnPoint(); ++iPoint) {
      ...
      and smooth wall : Inverse distance weighting ---*/
         ...
         node[iPoint]->SetRoughness_Height(config->GetRoughness_Height());
         ...
         if (variable_roughness){
           LE_Radius = 0.0158*chord;
           AoA_offset = LE_Radius*(config->GetAoA()*PI_NUMBER/180.0);
           Ac = config->GetAccumulation_Parameter();
           n0 = config->GetFreezing_Fraction();
           rough_lim = config->GetRoughness_Limit();
           smooth_lim = config->GetSmooth_Width();
           roughness_max = config->GetRoughness_Height();

           /*--- Retrieve coordinates of iPoint ---*/

           X_coord = node[iPoint]->GetCoord(0);
           Y_coord = node[iPoint]->GetCoord(1);
           if (nDim ==3)
           Z_coord = node[iPoint]->GetCoord(2);

           /*--- Retrieve coordinates of "wall neighbor" of iPoint" ---*/

           dist = node[iPoint]->GetWall_Distance();
           ...
           for(unsigned short iMarker=0; iMarker<config->GetnMarker_All(); ++iMarker) {
             if(((config->GetMarker_All_KindBC(iMarker) == HEAT_FLUX)  ||
                 ...
                 (config->GetMarker_All_Rough(iMarker) ) ) {
               for(unsigned long iVertex=0; iVertex<GetnVertex(iMarker); ++iVertex) {
                 unsigned long KKPoint = vertex[iMarker][iVertex]->GetNode();
                 su2double test_dist = 0.0;

                 for (unsigned short iDim = 0; iDim < nDim; iDim++)
                   test_dist += pow(node[iPoint]->GetCoord(iDim)-node[KKPoint]->GetCoord(iDim),2.0);

                   test_dist = sqrt(test_dist);

                 if (dist == test_dist){

                     X_wall_coord = node[KKPoint]->GetCoord(0);
                     Y_wall_coord = node[KKPoint]->GetCoord(0);
                     if (nDim == 3)
                     Z_wall_coord = node[KKPoint]->GetCoord(2);
                 }
               }
             }
           }
           ...
           if (nDim == 3) {
           /*--- Curvilinear abscissa of wall neighbor (origin at leading edge 0,0 ) ONLY FOR NACA0012 ---*/
           step = (X_wall_coord - 0.0)/10000;
           abs_curv = 0.0;

           for (unsigned long iStep = 0; iStep < 10000; iStep++){
             X_pos = (0.0 + (iStep+0.5)*step)/chord;
             if (Z_wall_coord > 0.0)
               abs_curv += step*sqrt(1.0 + pow(0.594689181*(0.1491113865/sqrt(X_pos+EPS)-0.127125232-0.715815812*X_pos+0.875954913*X_pos*X_pos-0.420698424*X_pos*X_pos*X_pos),2.0));
             else if (Z_wall_coord < 0.0)
               abs_curv -= step*sqrt(1.0 + pow(0.594689181*(0.1491113865/sqrt(X_pos+EPS)-0.127125232-0.715815812*X_pos+0.875954913*X_pos*X_pos-0.420698424*X_pos*X_pos*X_pos),2.0));
             else if (Z_wall_coord == 0.0)
               abs_curv += 0.0;
           }

           /*--- Adjustement with the stagnation point ---*/
             abs_curv += AoA_offset;
           }

           if (nDim == 2){
             abs_curv = X_wall_coord;
           }

           /*--- Parabolic roughness distribution (Han & Palacios, 2017) ---*/

           //roughness_height = max(config->GetRoughness_Min(), roughness_max + (-4.0*roughness_max/pow(rough_lim-smooth_lim, 2.0))*pow(abs(abs_curv)-(rough_lim+smooth_lim)/2.0, 2.0));

           /*--- Cubic roughness distribution  ---*/

           roughness_height = max(config->GetRoughness_Min(), roughness_max - 1.0/smooth_lim*pow(abs(abs(abs_curv)-rough_lim),3.0));

           ks = config->GetRoughness_Ratio()*roughness_height;

           node[iPoint]->SetRoughness_Height(roughness_height);
           node[iPoint]->SetRough(ks);
         }
    }
  }
